...
public class InMemoryRepositoryTest(CommonRepositoryFixture fixture) : IClassFixture<CommonRepositoryFixture>
{
    private readonly CommonRepositoryFixture _fixture = fixture;
    ...
    public void SaveList_Ok()
    {
        ...
        _fixture.CustomerRepo.Save(custs);
    ...
    public void Get_Linq_Ok()
    {
        ...
        var actual = _fixture.CustomerRepo.Get(x => x.Email.Equals(cust.Email));
    ...
    public void RemoveById_Ok()
    {
        ...
        _fixture.CustomerRepo.Save([.. Generator.Generate<Customer>(3)]);

        _fixture.CustomerRepo.Remove(2);

        Assert.Equal(2, _fixture.CustomerRepo.Count());

        var cust = _fixture.CustomerRepo.Get(2);
        Assert.Null(cust);
    }
    ...
    public void Query_Linq_Ok()
    {
        ...
        _fixture.CustomerRepo.Save([.. Generator.Generate<Customer>(3, x => x.Company = "ACME")]);
        _fixture.CustomerRepo.Save(Generator.Generate<Customer>(x => x.Company = "FooBar"));

        Assert.Equal(4, _fixture.CustomerRepo.Count());

        var actual = _fixture.CustomerRepo.Query(x => x.Company.Equals("ACME"));
